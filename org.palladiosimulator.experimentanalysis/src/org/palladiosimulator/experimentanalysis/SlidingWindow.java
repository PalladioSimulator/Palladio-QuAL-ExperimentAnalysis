 ...
 * Purpose of this window is to collect measurements of a certain kind that
 * occur within the window length. These measurements are written into the
 * window by a {@link SlidingWindowRecorder}.<br>
 * Once the window is full, the window moves forward by a fixed increment and
 * the collected measurements are propagated to all connected
 * {@link ISlidingWindowListener}s, e.g., certain
 * {@link SlidingWindowAggregator}s.<br>
 * Additionally, each window instance has an attached
 * {@link ISlidingWindowMoveOnStrategy} that defines how the collected data
 * (i.e., the measurements) is adjusted when the window moves forward.
 * 
...
public abstract class SlidingWindow extends AbstractObservable<ISlidingWindowListener> {
	...
	/**
	 * Initializes a new instance of the {@link SlidingWindow} class with the
	 * given parameters.
	 * 
	 * @param windowLength
	 *            The length of the window, given in any arbitrary
	 *            {@link Duration}. Additionally, this measure also denotes the
	 *            window increment.
	 * @param acceptedMetrics
	 *            As each window only accepts measurements that adhere to a
	 *            certain metric, a {@link MetricDescription} of must be
	 *            specified.
	 * @param moveOnStrategy
	 *            The {@link ISlidingWindowMoveOnStrategy} instance that defines
	 *            how the collected data (i.e., the measurements) is adjusted
	 *            when the window moves forward.
	 * @throws IllegalArgumentException
	 *             In one of the following cases:
	 *             <ul>
	 *             <li>given window length is negative</li>
	 *             <li>{@code acceptedMetrics} or {@code moveOnStrategy} is
	 *             {@code null}</li>
	 *             </ul>
	 ...
	 *            window is moved on, given in any arbitrary {@link Duration}.
	 * @param acceptedMetrics
	 *            As each window only accepts measurements that adhere to a
	 *            certain metric, a {@link MetricDescription} of must be
	 *            specified.
	...
	/**
	 * This method proves the validity of the arguments passed to either of the
	 * constructors.
	 * 
	 * @param windowLength
	 *            The length of the window, given in any arbitrary
	 ...
	 * @return {@code true} if the point in time value denoted by the given
	 *         measure is {@code != null, != infinite, != NaN} and {@code >= 0},
	 ...
	 * All attached {@link ISlidingWindowListener} are notified and the window
	 * moves on by the specified increment.
	 */
	...
	/**
	 * Notifies all attached observers that the sliding window is full, i.e.,
	 * invokes their respective
	 * {@link ISlidingWindowListener#onSlidingWindowFull(Iterable, Measure, Measure)}
	 * callback method.
	 */
	...
	/**
	 * Indicates whether the given measurements adheres to, that is, is
	 * compatible with this window's accepted metric.
	 * 
	 * @param measurement
	 *            A {@link MeasuringValue} instance.
	 * @return {@code true} if the measurement is compatible, otherwise
	 *         {@code false}.
	 */
	 ...
	 *             to the metric this window accepts.
	 */
	...
	/**
	 * This method is intended to be overridden by subclasses. They might do so
	 * to check their additional (potentially more specific) prerequisites.
	 * 
	 * @param newMeasurement
	...
	/**
	 * This method adds the given measurement to the window data. <b>No
	 * prerequisite checks are performed.</b><br>
	 * As this method is intended to be used by subclasses when they override
	 * {@link SlidingWindow#addMeasurement(MeasuringValue)}, implementations
	 * should take care to call
	 * {@link SlidingWindow#checkAddMeasurementPrerequisites(MeasuringValue)}
	 * beforehand to ensure uncorrupt window data.
	 * 
	 * @param newMeasurement
	...
	/**
	 * Discards the data (i.e., the measurements) currently collected by this
	 * window.
	 */
	 ...
	 * 
	 * @return A {@link MetricDescription} denoting the metric this window
	 *         accepts.
	...
	/**
	 * Gets whether the metric accepted by this window is a {@link BaseMetricDescription}.
	 * The same result is obtained by calling {@link #getAcceptedMetric()} and then doing a type check.
	...
	/**
	 * Gets the current, effective window length. Note that the effective window
	 * length might be smaller than specified in
	 * {@link SlidingWindow#SlidingWindow(Measure, MetricDescription, ISlidingWindowMoveOnStrategy)}
	 * or
	 * {@link SlidingWindow#SlidingWindow(Measure, Measure, MetricDescription, ISlidingWindowMoveOnStrategy)}
	 * .<br>
	 * 
	 * @return A {@link Measure} denoting the instantaneous effective window
	 *         length.
	 * @see SlidingWindow#getCurrentUpperBound()
	 */
	...
	/**
	 * Gets the value this window's lower bound is incremented by once the
	 * window moves on.
	 * 
	 * @return A {@link Measure} denoting the window increment.
	 * @see SlidingWindow#SlidingWindow(Measure, MetricDescription,
	 *      ISlidingWindowMoveOnStrategy)
	 * @see SlidingWindow#SlidingWindow(Measure, Measure, MetricDescription,
	 *      ISlidingWindowMoveOnStrategy)
	 */
	...
	/**
	 * Moves on, that is, the window's lower bound is incremented and the
	 * collected measurements are adjusted according to the specified
	 * {@link ISlidingWindowMoveOnStrategy}.
	 */
	private void moveOn() {
		adjustLowerBound();
	...
	/**
	 * Adjusts the window's lower bound, that increments it by the specified
	 * value.
	 */
	 ...
	 * @throws IllegalArgumentException
	 *             If the given listener is {@code null} or expects a metric
	 *             other than this window's accepted one.
	 ...
	 * 
	 * @return An <b>unmodifiable</b> list containing the attached
	 *         {@link ISlidingWindowListener}s.
	 */
