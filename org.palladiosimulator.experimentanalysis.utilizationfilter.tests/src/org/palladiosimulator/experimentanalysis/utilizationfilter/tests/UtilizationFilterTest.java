...
public class UtilizationFilterTest {
    ...
    public void setUp() throws Exception {
        ...
        this.defaultWindowLength = (Measure<Double, Duration>) this.filterUnderTestProperties.getDefaultConfiguration()
                .get(UtilizationFilterConfiguration.WINDOW_LENGTH_KEY);
        this.inputData = new MockDataSource(expectedInputDataMetric, this.defaultWindowLength);
        this.expectedUtilization = new TupleMeasurement(expectedOutputDataMetric, this.defaultWindowLength,
    ...
    public void testCanAccept() {
        IDataSource correctSource = this.inputData;
        assertTrue(this.filterUnderTest.canAccept(correctSource));

        // the utilization tuple metric is also a valid input metric ('overall utilization')
        correctSource = new MockDataSource(expectedOutputDataMetric, this.defaultWindowLength);
        assertTrue(this.filterUnderTest.canAccept(correctSource));
    }

    @Test
    public void testCanAcceptWrongMetric() {
        IDataSource wrongSource = new MockDataSource(MetricDescriptionConstants.AGGREGATED_COST_OVER_TIME,
                this.defaultWindowLength);
        assertFalse(this.filterUnderTest.canAccept(wrongSource));
    }
    ...
    public void testGetDataStream() {
        ...
        Measure<Double, Dimensionless> expected = this.expectedUtilization
                .getMeasureForMetric(MetricDescriptionConstants.UTILIZATION_OF_ACTIVE_RESOURCE);
        Measure<Double, Dimensionless> actual = measurement
                .getMeasureForMetric(MetricDescriptionConstants.UTILIZATION_OF_ACTIVE_RESOURCE);
    ...
    public void testUtilizationFilterCtorDatasourceWrongMetric() {
        IDataSource dataSource = new MockDataSource(MetricDescriptionConstants.EXECUTION_RESULT_METRIC_TUPLE,
                this.defaultWindowLength);
    ...
    private static class MockDataSource extends AbstractDataSource {
        ...
        private final List<MeasuringValue> data = new ArrayList<>();
        private final Measure<Double, Duration> windowLength;

        private MockDataSource(MetricDescription metric, Measure<Double, Duration> windowLength) {
            super(metric);
        ...
        private void initializeDataSource() {
            ...
            // no utilization next
            pointInTimeMeasure = Measure.valueOf(this.windowLength.doubleValue(SI.MILLI(SI.SECOND)) * 0.3,
                    SI.MILLI(SI.SECOND));
            ...
            pointInTimeMeasure = Measure.valueOf(this.windowLength.doubleValue(SI.CENTI(SI.SECOND)) * 0.6,
                    SI.CENTI(SI.SECOND));
            this.data.add(new TupleMeasurement(expectedInputDataMetric, pointInTimeMeasure, busyStateMeasure));
        ...
        public <M extends IMeasureProvider> IDataStream<M> getDataStream() {
            return new IDataStream<M>() {
                ...
                public Iterator<M> iterator() {
                    @SuppressWarnings("unchecked")
                    // that cast is type safe as Measurement implements the IMeasureProvider
                    // interface
