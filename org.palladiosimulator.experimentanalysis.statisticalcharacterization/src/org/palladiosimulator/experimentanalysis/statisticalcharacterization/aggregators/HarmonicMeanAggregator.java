...
/**
 * {@link SlidingWindowAggregator} which computes the geometric mean from the
 * measurements in the window once it moves on:<br>
 * It is equal to the reciprocal of the arithmetic mean of the reciprocals of
 * the measurements.<br>
 * Note that it is not defined if any of the measurements is exactly 0. In such
 * a case this implementation returns 0
 * 
...
public class HarmonicMeanAggregator extends StatisticalCharacterizationAggregator {
	...
	protected Measure<Double, Quantity> calculateStatisticalCharaterization(Iterable<MeasuringValue> windowData) {
		...
		double arithmeticMeanOfInverses = StreamSupport.stream(windowData.spliterator(), false)
				.map(measuringValue -> measuringValue.getMeasureForMetric(this.dataMetric))
				.collect(Collectors.averagingDouble(measure -> 1d / measure.doubleValue(this.dataDefaultUnit)));
